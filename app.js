...
function NewDefectPDF(generatedData) {
  try {
    ...
    fontFiles.forEach((fontFile) => {
      const fontData = fs.readFileSync(`${fontPath}${fontFile}`, {
        encoding: "base64",
      });
    ...
    const addStandardItemsToAllPages = () => {
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        doc.text(
          text,
          pageSize.width - xMargin - textWidth,
    ...
    const processTOC = () => {
      ...
      tocTable.forEach((toc, index) => {
        ...
        doc.textWithLink(finalText, baseXAxis, yAxis, {
          url: `#page=${currItemPage}`,
        });
        doc.textWithLink(
          `${currItemPage}`,
          baseXAxis + maxContentWidth,
          yAxis,
          { url: `#page=${currItemPage}` },
        );
        yAxis += 18;
    ...
    doc.setFont("OpenSans", "Bold");
    const coverTitle = `Demo a issue with Adobe Reader and jsPDF`;
    ...
    doc.text(
      `A simple test to show what is going wrong`,
      pageWidth / 2,
      pageHeight / 2 + 60,
      {
        align: "center",
    ...
    doc.text(
      `Last page wont work in Adobe Reader`,
      pageWidth / 2,
      pageHeight / 2 + 120,
      {
        align: "center",
    ...
    generatedData.paraObject.forEach((para) => {
      ...
      if (para.type === "string") {
        ...
        doc.setFont("OpenSans", "Regular");
        const contentTextLines = doc.splitTextToSize(
          para.content,
          maxContentWidth,
        );
